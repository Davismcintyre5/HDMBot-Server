"""
server/main.py — HDM BOT  |  Production Entry Point
"""
...
from server.whatsapp.connection import (
    build_client, send_text, BOT_NAME, get_prefix, set_prefix, _COMMANDS,
)
from server.whatsapp.client_manager import client_manager

from server.routes.auth_routes import bp as auth_bp, init_auth
from server.routes.session_routes import bp as session_bp
from server.routes.settings_routes import bp as settings_bp
from server.routes.command_routes import bp as command_api_bp
from server.routes.autoreply_routes import bp as autoreply_bp
from server.routes.contact_routes import bp as contact_bp
from server.routes.broadcast_routes import bp as broadcast_bp
from server.routes.chat_routes import bp as chat_bp

from server.services.session_service import session_service
from server.services.settings_service import settings_service as user_settings_service
...
def set_prefix_route():
    ...
    set_prefix(new_prefix[:3])
    return jsonify({"ok": True, "prefix": get_prefix()})
...
def _print_banner():
    ...
    table.add_row("AI Default", os.getenv("DEFAULT_AI_MODEL", "hdmai"))
    console.print(table)
    console.print(f"\n[bold green]✓[/bold green] [dim]API →[/dim] [cyan]http://{settings.FLASK_HOST}:{settings.FLASK_PORT}[/cyan]")
    console.print(f"[dim]{'─'*50}[/dim]\n")

def _run_flask():
    console.print(f"[bold blue][HTTP][/bold blue] Production + WebSocket on http://{settings.FLASK_HOST}:{settings.FLASK_PORT}")
...
def main():
    ...
    def on_any_message(client, msg, session_id):
        from server.whatsapp.connection import handle_message
